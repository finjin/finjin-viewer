...
float BoolToScaledFloat(bool value)
{
    float result = value; //0.0f or 1.0f
...
float3 ComputeDirectionalLight(LightData L, MaterialData mat, float3 pos, float3 normal, float3 toEye);
float3 ComputeDirectionalLight(LightData L, MaterialData mat, float3 pos, float3 normal, float3 toEye)
{
    float3 diffuseLightColor(0, 0, 0);
